...
- **Commands** — one file that works as slash, prefix, or both (`docs/commands`)
- **Components** — buttons, modals, and dropdowns with compact signed custom ids (`docs/components`)
- **Events** — typed listeners with `once` support (`docs/events`)
- **Config & errors** — `defineConfig` + zod env + customizable messages (`docs/config`)
- **Databases** — none / file / sqlite / postgres / mysql / mongo / redis with automatic cooldowns (`docs/database`)
- **CLI** — `create`, `add`, `update`, `doctor`, `env`, `sync` (`docs/cli`)
